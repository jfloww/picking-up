...
One item in this register requires direct repository-owner input:
...
4. Resolve RF-017 so routine occurrence creation has one idempotent server and
   database authority.
5. RF-006 is now resolved (both rounds); its former line item here — cross-
   field domain rules alongside each command — landed as `TaskSerializer`
   validation plus two portable DB constraints instead. See the
   [round 2 resolution](2026-08-06-domain-validation.md) for what did and
   didn't get database-level enforcement and why.
